...
public class UserAction extends BaseAction<User> {
	...
	public String login() throws Exception {
		System.out.println("档案管理登录验证开始"+ticket);
...
//			String dateStr = DateUtil.getNow("yyyy-MM-dd")+SystemTool.getWindowsMACAddress();
			...
			if(null != user){
				...
				if(password.equals(user.getPassword())){
					boolean ishas = (userService.checkPrivilege(user, "档案管理_查看")||userService.checkPrivilege(user, "档案管理_编辑"));
					if(ishas){
						...
						System.out.println("档案管理登录验证成功"+ticket);
						//写日志
						writeLog("登录系统","",user);
						return "toIndex";
					}
		...
		System.out.println("档案管理登录验证失败"+ticket);
		return "toLogout";
	}
	/**修改密码*/
