/****开始********************************************************/
	...
	function newOneSOP(addOrEdit){
		...
		if(addOrEdit==1)//新建
		{
			...
			}else if(row.archiveCode==null||row.archiveCode==''){
				$.messager.alert("提示框","该SOP没有入档,不可追加！");
			}else if(row.isInvalid==true){
	...
	function setStatus2(addOrEdit){
		if(addOrEdit==1||addOrEdit==2||addOrEdit==3){
			...
			$('#chooseOneArchiveTypeCode2').linkbutton('enable');
			$('#searchNotArchiveSOP').linkbutton('enable');
			...
			$('#chooseOneArchiveTypeCode2').linkbutton('disable');
			$('#searchNotArchiveSOP').linkbutton('disable');
	...
	function searchNotArchiveSOPByCode()
	{
		var sopcode = $('#sopcode').val();
		var sopflag = $('#sopflag').val();
		 $.ajax({
	 	 	  	url : sybp()+'/tblFileContentSOPAction_getNotArchiveByCode.action?sopcode='+sopcode+'&sopflag='+sopflag,
	 		  	type: 'post',
	 		  	contentType: "application/x-www-form-urlencoded; charset=utf-8", 
	 		  //	data: $('#oneTblFileContentSOPForm').serialize(),
	 		  	...
	 		  	success:function(r){
				 	...
				 	{
				 		/* 显示Dialog */
						document.getElementById("chooseNotArchiveSOPDialog2").style.display="block";
				 		$('#chooseNotArchiveSOPDataGrid').treegrid('loadData',r);
				 		
				 		//$('#chooseNotArchiveSOPDialog').dialog('setTitle','选择一个未入档的SOP');
						$('#chooseNotArchiveSOPDialog').dialog('open'); 
	...
	function chooseOneNotArchiveSOP()
	{
		var r = $('#chooseNotArchiveSOPDataGrid').treegrid('getSelected');
		if(r!=''&&r!=null)
		{
			$('#chooseNotArchiveSOPDialog').dialog('close');
			
			$('#oneFileRecordId2').val(r.fileRecordId);
			$('#sopver').val(r.sopver);
			
			$('#sopver').attr('readOnly',true);
			$('#sopname').attr('readOnly',true);
			$('#soptypeCode').attr('disabled',true);
			...
			$.messager.alert('提示框','请选择一个未入档的SOP！');
		}
	}
 		  ...
 		  //	data: $('#oneTblFileContentSOPForm').serialize(),
 		  	...
 		  	success:function(r){
			 	if(r&&r.success)
			 	{
			 		$('#sopver').attr('readOnly',false);
					$('#sopname').attr('readOnly',false);
					$('#soptypeCode').attr('disabled',false);
		        		  	    	 ...
		        		  	    	 //contentWind.$('#searchRecordButton').click();
			        		  	    	...
			        		  	    	dg.treegrid('select',r.record.fileRecordId);
			        		  	    	  ...
			        		  	    	  if($('#continueAddSOPButton').attr('checked')!='checked')
			        		  	    	  {
				        		  			...
				        		  			//	$('#oneArchiveTypeName2').val('');
				        						$.ajax({
				        		  		 	 	  	url : sybp()+'/tblFileContentStudyAction_getMaxArchiveCode.action?archiveTypeCode='+$('#oneArchiveTypeCode2').val(),
				        		  		 		  	contentType: "application/x-www-form-urlencoded; charset=utf-8", 
				        		  		 		  	dataType:'json',
				        		  		 		  	success:function(r){
				        		  		 		  	      if(r&&r.archiveCode){
...
//   		  	    	  contentWind.$('#searchRecordButton').click();
	...
	function searchSOPRecord(isAll,isNowValid,isInvalid,needChange,changeEndDate,yearNum,yearNumUnit,isFileDate,start,end,keepEndDate,isDestory,isValid,searchString)
	{
		...
		if(childWind.$){
			...
			$.ajax({
				url : sybp()+'/tblFileContentSOPAction_loadList.action?isAll='+isAll+'&isNowValid='+isNowValid+'&isInvalid='+isInvalid+'&needChange='+needChange+'&changeEndDate='+changeEndDate+'&yearNum='+yearNum+'&yearNumUnit='+yearNumUnit+'&fileStartDate='+start+'&fileEndDate='+end+'&keepEndDate='+keepEndDate+'&isDestory='+isDestory+'&isValid='+isValid,
	   		  	type: 'post',
	   		  	contentType: "application/x-www-form-urlencoded; charset=utf-8", 
	   		  	data: {
