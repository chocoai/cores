...
<script language="javascript" src="${pageContext.request.contextPath}/script/javascript/quarantineAction/virusMAddEdit.js" charset="utf-8"></script>
<script type="text/javascript">
    ...
    function showVirusMTable(names){
    	$('#virusMTable').datagrid({
			url : sybp()+'/virusMAction_loadList.action?name='+names,
			title:'',
			height:tableHeight,
			width:tableWidth,
			...
			onSelect:function(rowIndex, rowData){
    		  $('#addVirusMButton').linkbutton('enable');
    		  $('#editVirusMButton').linkbutton('enable');
    		  $('#deleteVirusMButton').linkbutton('enable');
			},
			onLoadSuccess:function(data){
			   $('#addVirusMButton').linkbutton('enable');
	    	   $('#editVirusMButton').linkbutton('disable');
	           $('#deleteVirusMButton').linkbutton('disable');
	           var selectid=$('#vmid').val();
	           if(selectid != ""){
			          for(var i = 0 ; i< data.rows.length;i++){
			             if(selectid == data.rows[i].id){
			            	$('#virusMTable').datagrid('selectRow',i);
			             }
			          }
			   }
			},
    ...
    function onSearchButton(){
    	var name =  $('#virusmname').val();
    	showVirusMTable(name);
    }
	//添加按钮事件
    function onAddButton(){
    	showVirusMAddEditDialog('afterAddDialog','add','添加病毒记录');
    }
    
	//	添加Dialog后事件
    function afterAddDialog(){
       parent.parent.showMessager(1,'添加成功',true,5000);
       $('#virusMTable').datagrid('reload');
    }
    //wan 编辑按钮事件
    function onEditButton(){
         var row= $('#virusMTable').datagrid('getSelected');
        if(row != null ){
        	showVirusMAddEditDialog('afterEditDialog','edit','编辑病毒记录');
        }else{
   ...
   //删除按钮事件(数据库还保存)
     ...
     //根据Id删除相应员工(数据库还保存)
       function delVirusM(id){
   	   	$.ajax({
   			url:sybp()+'/virusMAction_delVirus.action',
   			type:'post',
   			data:{id:id},
   			dataType:'json',
   			success:function(r){
...
<div id="layoutMainDiv" class="easyui-layout"  style="width:100%;height:100%; display:none;"> 
 	<div region="center" title="" style="overflow: hidden;">
 	 	<!-- <div id="tabs" class="easyui-tabs" fit="true" border="false">-->
            ...
            <div style="height:30px;">
			       <div style="float:left; width:100%;padding-top:5px;height:22px;">
			    		&nbsp;&nbsp;病毒检疫方法名称
			    		<input id="virusmname" type="text"    style="width:100px;"></input>
			    	    <!-- &nbsp;&nbsp;&nbsp;&nbsp; 出场日期
		                &nbsp;
        ...
        <!-- <a id="addCustomerButton" class="easyui-linkbutton" onclick="onAddButton();" data-options="iconCls:'icon-add',plain:true">添加</a> -->
			<a id="addVirusMButton" class="easyui-linkbutton" onclick="onAddButton();"  data-options="iconCls:'icon-groupadd',plain:true,disabled:true">添加</a>
			<a id="editVirusMButton" class="easyui-linkbutton" onclick="onEditButton();" data-options="iconCls:'icon-groupedit',plain:true,disabled:true">编辑</a>
			<a id="deleteVirusMButton" class="easyui-linkbutton" onclick="onDeleteButton()" data-options="iconCls:'icon-groupdelete',plain:true,disabled:true">删除</a>
    ...
    <%@include file="/WEB-INF/jsp/quarantineAction/virusMAddEdit.jspf" %>
</div>
</body>
</html>