...
<script language="javascript" src="${pageContext.request.contextPath}/script/javascript/quarantineAction/parasiteMAddEdit.js" charset="utf-8"></script>
<script type="text/javascript">
    ...
    function showParasiteMTable(names){
    	$('#parasiteMTable').datagrid({
			url : sybp()+'/parasiteMAction_loadList.action?name='+names,
			title:'',
			height:tableHeight,
			width:tableWidth,
			...
			onSelect:function(rowIndex, rowData){
    		  $('#addParasiteMButton').linkbutton('enable');
    		  $('#editParasiteMButton').linkbutton('enable');
    		  $('#deleteParasiteMButton').linkbutton('enable');
			},
			onLoadSuccess:function(data){
			   $('#addParasiteMButton').linkbutton('enable');
	    	   $('#editParasiteMButton').linkbutton('disable');
	           $('#deleteParasiteMButton').linkbutton('disable');
	           var selectid=$('#pmid').val();
	           if(selectid != ""){
			          for(var i = 0 ; i< data.rows.length;i++){
			             if(selectid == data.rows[i].id){
			            	$('#parasiteMTable').datagrid('selectRow',i);
			             }
			          }
			   }
			},
    ...
    function onSearchButton(){
    	var name =  $('#parasitemname').val();
    	showParasiteMTable(name);
    }
	//添加按钮事件
    function onAddButton(){
    	showParasiteMAddEditDialog('afterAddDialog','add','添加寄生虫检疫方法记录');
    }
    
	//	添加Dialog后事件
    function afterAddDialog(){
       parent.parent.showMessager(1,'添加成功',true,5000);
       $('#parasiteMTable').datagrid('reload');
    }
    //wan 编辑按钮事件
    function onEditButton(){
         var row= $('#parasiteMTable').datagrid('getSelected');
        if(row != null ){
        	showParasiteMAddEditDialog('afterEditDialog','edit','编辑寄生虫检疫方法记录');
        }else{
           $.messager.alert('提示','请选择编辑的寄生虫检疫方法记录!');
        }
    }
   //编辑后事件
   ...
   //删除按钮事件(数据库还保存)
     ...
     //根据Id删除相应员工(数据库还保存)
       function delParasiteM(id){
   	   	$.ajax({
   			url:sybp()+'/parasiteMAction_delParasite.action',
   			type:'post',
   			data:{id:id},
   			dataType:'json',
   			success:function(r){
...
<body>
<s:hidden id="pmid" name="pmid"></s:hidden>
...
<div id="layoutMainDiv" class="easyui-layout"  style="width:100%;height:100%; display:none;"> 
 	<div region="center" title="" style="overflow: hidden;">
 	 	<!-- <div id="tabs" class="easyui-tabs" fit="true" border="false">-->
            ...
            <div style="height:30px;">
			       <div style="float:left; width:100%;padding-top:5px;height:22px;">
			    		&nbsp;&nbsp;寄生虫检疫方法名称:
			    		<input id="parasitemname" type="text"    style="width:100px;"></input>
			    	    <!-- &nbsp;&nbsp;&nbsp;&nbsp; 出场日期
		                &nbsp;
        ...
        <!-- <a id="addCustomerButton" class="easyui-linkbutton" onclick="onAddButton();" data-options="iconCls:'icon-add',plain:true">添加</a> -->
			<a id="addParasiteMButton" class="easyui-linkbutton" onclick="onAddButton();"  data-options="iconCls:'icon-groupadd',plain:true,disabled:true">添加</a>
			<a id="editParasiteMButton" class="easyui-linkbutton" onclick="onEditButton();" data-options="iconCls:'icon-groupedit',plain:true,disabled:true">编辑</a>
			<a id="deleteParasiteMButton" class="easyui-linkbutton" onclick="onDeleteButton()" data-options="iconCls:'icon-groupdelete',plain:true,disabled:true">删除</a>
    ...
    <%@include file="/WEB-INF/jsp/quarantineAction/parasiteMAddEdit.jspf" %>
</div>
</body>
</html>