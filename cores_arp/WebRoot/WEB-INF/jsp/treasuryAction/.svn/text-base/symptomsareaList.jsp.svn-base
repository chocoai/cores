...
<script language="javascript" src="${pageContext.request.contextPath}/script/javascript/treasuryAction/symptomsareaAddEdit.js" charset="utf-8"></script>
<script type="text/javascript">
	...
	$(function(){
		...
		//初始化房舍信息表
		showSymptomsareaTable('');
		 //end treegrid
		
		//显示整个布局
		$('#layoutMainDiv').css('display','');
    });//匿名函数结束
    function showSymptomsareaTable(names){
    	$('#symptomsareaTable').datagrid({
			url : sybp()+'/symptomsareaAction_loadList.action?name='+names,
			title:'',
			height:tableHeight,
			width:tableWidth,
			...
			onSelect:function(rowIndex, rowData){
    		  $('#addSymptomsareaButton').linkbutton('enable');
    		  $('#editSymptomsareaButton').linkbutton('enable');
    		  $('#deleteSymptomsareaButton').linkbutton('enable');
			},
			onLoadSuccess:function(data){
			   $('#addSymptomsareaButton').linkbutton('enable');
	    	   $('#editSymptomsareaButton').linkbutton('disable');
	           $('#deleteSymptomsareaButton').linkbutton('disable');
	           var selectid=$('#sssid').val();
	           if(selectid != ""){
			          for(var i = 0 ; i< data.rows.length;i++){
			             if(selectid == data.rows[i].id){
			            	$('#symptomsareaTable').datagrid('selectRow',i);
			             }
			          }
			   }
			},
    ...
    function onSearchButton(){
    	var name =  $('#sysareaname').val();
    	showSymptomsareaTable(name);
    }
	//添加按钮事件
    function onAddButton(){
    	showSymptomsareaAddEditDialog('afterAddDialog','add','添加疾病系统');
    }
    
	//	添加Dialog后事件
    function afterAddDialog(){
       parent.parent.showMessager(1,'添加成功',true,5000);
       $('#symptomsareaTable').datagrid('reload');
    }
    //wan 编辑按钮事件
    function onEditButton(){
         var row= $('#symptomsareaTable').datagrid('getSelected');
        if(row != null ){
        	showSymptomsareaAddEditDialog('afterEditDialog','edit','编辑疾病系统');
        }else{
           $.messager.alert('提示','请选择编辑的疾病系统!');
        }
    }
   //编辑后事件
   ...
   //删除按钮事件(数据库还保存)
     ...
     //根据Id删除相应员工(数据库还保存)
       function delSymptomsarea(id){
   	   	$.ajax({
   			url:sybp()+'/symptomsareaAction_delSymptomsarea.action',
   			type:'post',
   			data:{id:id},
   			dataType:'json',
   			success:function(r){
...
<body>
<s:hidden id="sssid" name="sssid"></s:hidden>
...
<div id="layoutMainDiv" class="easyui-layout"  style="width:100%;height:100%; display:none;"> 
 	<div region="center" title="" style="overflow: hidden;">
 	 	<!-- <div id="tabs" class="easyui-tabs" fit="true" border="false">-->
            ...
            <div style="height:30px;">
			       <div style="float:left; width:100%;padding-top:5px;height:22px;">
			    		&nbsp;&nbsp;疾病系统
			    		<input id="sysareaname" type="text"    style="width:100px;"></input>
			    	    <!-- &nbsp;&nbsp;&nbsp;&nbsp; 出场日期
		                &nbsp;
        ...
        <!-- <a id="addCustomerButton" class="easyui-linkbutton" onclick="onAddButton();" data-options="iconCls:'icon-add',plain:true">添加</a> -->
			<a id="addSymptomsareaButton" class="easyui-linkbutton" onclick="onAddButton();"  data-options="iconCls:'icon-groupadd',plain:true,disabled:true">添加</a>
			<a id="editSymptomsareaButton" class="easyui-linkbutton" onclick="onEditButton();" data-options="iconCls:'icon-groupedit',plain:true,disabled:true">编辑</a>
			<a id="deleteSymptomsareaButton" class="easyui-linkbutton" onclick="onDeleteButton()" data-options="iconCls:'icon-groupdelete',plain:true,disabled:true">删除</a>
    ...
    <%@include file="/WEB-INF/jsp/treasuryAction/symptomsareaAddEdit.jspf" %>
</div>
</body>
</html>