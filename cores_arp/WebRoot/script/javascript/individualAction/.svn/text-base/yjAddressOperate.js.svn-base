function showYJAddressOperaterDialog(title){
	/* 显示职位主Dialog */
	...
	onDblClickRow:function(rowIndex, rowData){//鼠标双击触发.
		showAddressAddEditDialog('afterEditAddressDialog','view','查看引进地信息');
	}
	});
  
}
function onaddAddressButton(){
	showAddressAddEditDialog('afterAddAddressDialog','add','添加引进地');
}
...
function oneditAddressButton(){
	showAddressAddEditDialog('afterEditAddressDialog','edit','编辑引地');
}
...
function ondelAddressButton(){
	 var row= $('#addressTable').datagrid('getSelected');
 	 if(null != row ){
      	$.messager.confirm('确认对话框', '确认删除此引进地？', function(r){
				if (r){
					delAddress(row.id);
       ...
       $.messager.alert('提示','请选择要删除的引进地!');
     }
}
function delAddress(id){
   	$.ajax({
		url:sybp()+'/individualAction_delAddress.action',
		type:'post',
		data:{id:id},
		dataType:'json',
		success:function(r){
			if(r && r.success){
				parent.parent.showMessager(1,'引进地删除成功',true,5000);
				 $('#addressTable').datagrid('reload');
			}else{
				parent.parent.showMessager(2,'引进地删除失败',true,5000);
			}
		}
	});
} 
function showAddressAddEditDialog(clickName,addOrEdit,title){
	...
	if(addOrEdit == "add"){
		...
		var row= $('#addressTable').datagrid('getSelected');
		$.ajax({
    		url:sybp()+'/individualAction_toEditAddress.action',
    		type:'post',
    		cache:false,
    		data:{
			 id:row.id
...
function onAddressDialogSaveButton(){
		if( $('#addressAddEditForm').form('validate')){
			...
			if($('#addOrEditDis').val() == 'add'){
				$.ajax({
					url:sybp()+'/individualAction_addAddress.action',
					type:'post',
				...
				$.ajax({
					url:sybp()+'/individualAction_editSaveAddress.action',
					type:'post',
					data:$('#addressAddEditForm').serialize(),
					dataType:'json',
					success:function(r){
