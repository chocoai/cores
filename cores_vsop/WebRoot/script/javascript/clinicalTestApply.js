...
function testPhaseCheck(){
	...
	if(testPhase==2||testPhase==3||testPhase==4||testPhase==7){
		if(testPhaseNum == ""){
			return false;
		}
	}
			...
			$.ajax({
				...
				if(r && r.success){
					...
					window.location.href="${pageContext.request.contextPath}/tblClinicalTestReqAction_ireport.action?studyNoPara="
						+encodeURIComponent(r.obj.studyNo)+"&reqNoPara="+r.obj.reqNo+"&toWhere=apply" ;//
				}
			}
			});
	    } 
...
function outPort3(){
	...
	parent.parent.showMessager(2,'数据加载中...',true,5000);
	window.location.href="${pageContext.request.contextPath}/tblClinicalTestReqAction_ireport.action?studyNoPara="
		+encodeURIComponent($('#studyNo').val())+"&reqNoPara="+$('#reqNo').val()+"&toWhere=view" ;
}
/**
 * 打印预览(list页面)
 * @return
...
function outPort2(studyNoPara, reqNoPara){
	...
	parent.parent.showMessager(2,'数据加载中...',true,5000);
	window.location.href="${pageContext.request.contextPath}/tblClinicalTestReqAction_ireport.action?studyNoPara="
		+encodeURIComponent(studyNoPara)+"&reqNoPara="+reqNoPara;
}
...
function eventAfterSign(){
	window.location.href="${pageContext.request.contextPath}/tblClinicalTestReqAction_sign.action?studyNoPara="+encodeURIComponent($('#studyNo').val())+"&reqNoPara="+$('#reqNo').val();
}
//签字后提交
...
function clinicalTestApply(){
	window.location.href= "tblClinicalTestReqAction_clinicalTestApply.action?parentReqNo1="+$('#connectionNumber').val()+"&studyNoPara="+encodeURIComponent($('#studyNoPara').val());
}