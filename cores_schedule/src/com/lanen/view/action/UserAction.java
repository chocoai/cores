...
public class UserAction extends BaseAction<User> {
	...
	public String login() throws Exception {
		System.out.println("日程管理登录验证开始"+ticket);
...
//			String dateStr = DateUtil.getNow("yyyy-MM-dd")+SystemTool.getWindowsMACAddress();
			...
			if(null != user){
				...
				if(password.equals(user.getPassword())){
					boolean ishas = userService.checkPrivilege(user, "综合管理-登录");
					if(ishas){
						...
						System.out.println("日程管理登录验证通过"+userName);
						//写日志
						writeLog("登录系统","",user);
						return "toIndex";
					}
		...
		System.out.println("日程管理登录验证失败"+ticket);
		return "toLogout";
	}
	/**修改密码*/
