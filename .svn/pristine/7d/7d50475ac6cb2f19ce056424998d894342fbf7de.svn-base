...
//		// String[] _nory_changes
//		// ={"id","resKind","resName","validFlag","parentId","orderNo"};
//		// String jsonStr = JsonPluginsUtil.beanListToJson(objList,
//		// _nory_changes , true);
//		List<TblAnimalHouse> list;
//		if (null != model.getParentId()) {
//			list = tblAnimalHouseService.getParentId(model.getParentId());
//		} else {
//			list = tblAnimalHouseService.getresKind(model.getResKind());
//		}
//		String jsonStr = "";
//		jsonStr += "[";
//		for (TblAnimalHouse houes : list) {
//			jsonStr += "{";
//			// ss +=
//			// "\"id\": \""+category.getId()+"\",\"text\": \""+category.getName().toString()+"\",\"iconCls\": \"icon-ok\",\"state\": \"closed\"";;
//			int resKind = houes.getResKind(); // 判断当前节点是否还有子节点
//			if (resKind == 3) { // 没有子节点 设置 state 为空
//				jsonStr += String
//						.format(
//								"\"id\": \"%s\", \"text\": \"%s\", \"iconCls\": \"icon-home\", \"state\": \"\"",
//								houes.getId(), houes.getResName());
//			} else if (resKind == 1) { // 还有子节点 设置 state为closed
//				jsonStr += String
//						.format(
//								"\"id\": \"%s\", \"text\": \"%s\", \"iconCls\": \"icon-home\", \"state\": \"closed\"",
//								houes.getId(), houes.getResName());
...
//	}
	...
	public String addUI() throws Exception {
		...
		if (null != parentId && !parentId.equals("")) {
			...
			TblAnimalHouse animalHouse = tblAnimalHouseService
					.getById(parentId);
	...
	public void CheckResKind() {
		...
		if (null != model.getParentId() && !"".equals(model.getParentId())) {
			TblAnimalHouse animalHouse = tblAnimalHouseService.getById(model
					.getParentId());
			int resKind = animalHouse.getResKind();
			if (resKind != 3) {
	...
	public void checkresName() {
		if (null != model.getResName() && !"".equals(model.getResName())) {
			...
			if (null != pid && !pid.equals("")) {
				isExist = tblAnimalHouseService.isExistByResNameAndPid(model
						.getResName(), pid);
			} else {
				isExist = tblAnimalHouseService.isExistByResName(model
						.getResName());
			}
	...
	public void checkOtherresName() {
		if (null != model.getResName() && !"".equals(model.getResName())
			...
			if (oldresName.trim().equals(model.getResName().trim())) {
				...
				boolean isExist;
				if (null != pid && !pid.equals("")) {
					isExist = tblAnimalHouseService.isExistByResNameAndPid(
							model.getResName(), pid);
				} else {
					isExist = tblAnimalHouseService.isExistByResName(model
							.getResName());
				}
	...
	public void beforDelete(){
		Json json = new Json();
		boolean falg= tblAnimalHouseService.referencedRid(model.getId());
		if(falg){
			json.setSuccess(true);
		}else{
			json.setMsg("房间已引用，无法删除！");
		}
	...
	public void delete() {
		...
		if(falg){
			...
			if (null != model.getId() && !"".equals(model.getId())) {
				...
				List<TblAnimalHouse> houselist = tblAnimalHouseService
				.getParentId(model.getId());
...
//		int resKind = obj.getResKind();
//		if (resKind == 1) {
//			map.put("resname", obj.getResName());
//		} else if (resKind == 2) {
//			TblAnimalHouse house = tblAnimalHouseService.getById(obj
//					.getParentId());
//			map.put("resname", house.getResName() + " " + obj.getResName());
//		} else if (resKind == 3) {
//			TblAnimalHouse house = tblAnimalHouseService.getById(obj
//					.getParentId());
//			TblAnimalHouse house1 = tblAnimalHouseService.getById(house
//					.getParentId());
//			map.put("resname", house1.getResName() + " " + house.getResName()
//					+ "  " + obj.getResName());
//		}
//		map.put("success", true);
//		map.put("resid", model.getId());
//		String json = JsonPluginsUtil.beanToJson(map);
//		writeJson(json);
//	}
