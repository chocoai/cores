...
//							parent.parent.showMessager(2,'删除失败',true,5000);
	 ...
	 var attachedViscera=row.attachedViscera;
	 var attachedVisceras = new Array();
	 //在复制解剖申请-脏器称重-附加脏器前先清空数据
	 ...
	 if(rowsLength==0){ 
		//dialog中更新附加脏器
			$('#pathPlanVisceraWeighTable1').datagrid('updateRow',{
				 index: rowIndex,	
				   row: {
				          attachedViscera:node.visceraName
			          }
			 });
...
//			 });
		...
		//dialog中更新附加脏器
			$('#pathPlanVisceraWeighTable1').datagrid('updateRow',{
				 index: rowIndex,	
				   row: {
				          attachedViscera:attachedViscera+'、'+node.visceraName
			          }
			 });
			$('#pathPlanVisceraWeighTable1').datagrid('selectRow',rowIndex);
			//页面中更新附加脏器
