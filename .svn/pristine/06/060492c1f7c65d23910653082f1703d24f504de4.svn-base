...
@Controller
@Scope("prototype")
public class DictHematAction  extends BaseAction<DictHemat>{
	...
	public void loadList(){
		List<DictHemat>  objList=dictHematService.getAll();
	...
	public void checkName(){
		if(null!=model.getName() && !"".equals(model.getName())){
			boolean isExist = dictHematService.isExistByName(model.getName());
			if(!isExist){
				writeJson("true");
			}else{
				writeJson("false");
	...
	public void checkAbbr(){
		if(null!=model.getAbbr() && !"".equals(model.getAbbr())){
			boolean isExist = dictHematService.isExistByAbbr(model.getAbbr());
			if(!isExist){
				writeJson("true");
			}else{
				writeJson("false");
	...
	public String add() throws Exception {
		...
		model.setOrderNo(dictHematService.getNextOrderNo());
		dictHematService.save(model);
		return "toList";
	}
	...
	public void checkIsExist()throws Exception{
		...
		if(null != abbr && !"".equals(abbr) ){
			boolean isExist = passagewayService.isExist(abbr,2);
			if(isExist){
	...
	public void delete() {
		Json json = new Json();
		if(null!=model.getName() && !"".equals(model.getName())){
			DictHemat  dictHemat  = dictHematService.getById(model.getName());
			String abbr = dictHemat.getAbbr();
			boolean isExist = passagewayService.isExist(abbr,2);
			if(isExist){
				json.setSuccess(false);
				json.setMsg("‘"+abbr+"’已设置通道号，无法删除");
			}else{
				dictHematService.delete(model.getName());
				json.setSuccess(true);
				json.setMsg("删除成功");
			}
		}else{
	...
	public String editUI() throws Exception {
		if(null!=model.getName() && !"".equals(model.getName())){
			DictHemat obj  =dictHematService.getById(model.getName());
...
//			ActionContext.getContext().getValueStack().push(model);
		...
		DictHemat DictHemat =dictHematService.getById(model.getName());
		DictHemat.setAbbr(model.getAbbr());
		DictHemat.setPrecision(model.getPrecision());
		DictHemat.setUnit(model.getUnit());
		dictHematService.update(DictHemat);
		return "toList";
	}
	...
	public boolean checkValue(){
		...
		boolean isExistName = dictHematService.isExistByName(model.getName());
		if(isExistName){
			return false;
		}
		boolean isExistAbbr = dictHematService.isExistByAbbr(model.getAbbr());
		if(isExistAbbr){
			return false;
		}
		return true;
	...
	public boolean checkValue_edit(){
		...
		boolean isExistAbbr = dictHematService.isExistByNameAbbr(model.getName(),model.getAbbr());
		if(isExistAbbr){
			return false;
		}
		return true;
	...
	public void moveOrder() {
		...
		if(orderNoPara!=0&&orderNoNext!=0){
			dictHematService.moveOeder(orderNoPara, orderNoNext);
			map.put("success", true);
			map.put("msg","移动设置成功");
			DictHemat currentRow = dictHematService.getByOrderNo(orderNoPara);
			DictHemat nextRow = dictHematService.getByOrderNo(orderNoNext);
