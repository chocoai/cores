...
public class UserAction extends BaseAction<User> {
	...
	public String login() throws Exception {
		System.out.println("系统审计登录验证开始"+ticket);
...
//			String dateStr = DateUtil.getNow("yyyy-MM-dd")+SystemTool.getWindowsMACAddress();
			System.out.println(ticket.substring(32));
			...
			if(null != user){
				...
				if(password.equals(user.getPassword())){
					boolean ishas = userService.checkPrivilege(user, "系统审计_登录");
					if(ishas){
						...
						System.out.println("系统审计登录验证通过"+userName);
						//写日志
						writeLog("登录系统","",user);
						return "toIndex";
					}
		...
		System.out.println("系统审计登录验证失败"+ticket);
		return "toLogout";
	}
	/**修改密码*/
...
//		Map<String,Object> map = userService.checkPrivilege(model.getUserName(), model.getPassword(),"综合管理-登录","系统设置登录");
//		//判断用户是否为空
...
//	}
	...
	public void fupasswordCheck() {
		...
		if (null != tempUser) {
			...
			if (flag && null != tempUser && null != model.getPassword()
					&& !"".equals(model.getPassword())) {
				if (tempUser.getPassword().equals(
						DigestUtils.md5Hex(model.getPassword()))) {
					json.setSuccess(true);
					json.setMsg("密码验证通过");
