 ...
 * @version 20111221
 * 
 *          {@code 现使用json-lib组件实现 需要 json-lib-2.4-jdk15.jar ezmorph-1.0.6.jar
 *          commons-collections-3.1.jar commons-lang-2.0.jar 支持 * }
 */
...
public class JsonPluginsUtil {
	...
	public static String beanToJson(Object bean,String datePattern) {
	...
	//黄后添加
	public static String beanToJson(Object bean,String datePattern, String[] _nory_changes,
			boolean nory) {
		...
		if (nory) {// 转换_nory_changes里的属性
			...
			json = JSONObject.fromObject(bean, configJson(str.split(":"),datePattern));
		} else {// 转换除了_nory_changes里的属性
			json = JSONObject.fromObject(bean, configJson(_nory_changes,datePattern));
		}
		return json.toString();
	}
	//    String[]和日期（黄后添加）
	...
	@SuppressWarnings("unchecked")
	public static String beanListToJson(List beans, String datePattern) {
		StringBuffer rest = new StringBuffer();
		rest.append("[");
		int size = beans.size();
		for (int i = 0; i < size; i++) {
			rest.append(beanToJson(beans.get(i),datePattern) + ((i < size - 1) ? "," : ""));
		}
		rest.append("]");
		return rest.toString();
	}
	...
	@SuppressWarnings("unchecked")
	public static String beanListToJson(List beans, String datePattern, String[] _nory_changes,
			boolean nory) {
		StringBuffer rest = new StringBuffer();
		rest.append("[");
		int size = beans.size();
		for (int i = 0; i < size; i++) {
			try {
				rest.append(beanToJson(beans.get(i),datePattern, _nory_changes, nory));
				if (i < size - 1) {
					rest.append(",");
				}
			} catch (Exception e) {
