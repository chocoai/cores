...
@Controller
@Scope("prototype")
public class DictBioChemAction  extends BaseAction<DictBioChem>{
	...
	public void loadList(){
		List<DictBioChem>  objList=dictBioChemService.getAll();
	...
	public void checkName(){
		if(null!=model.getName() && !"".equals(model.getName())){
			boolean isExist = dictBioChemService.isExistByName(model.getName());
			if(!isExist){
				writeJson("true");
			}else{
				writeJson("false");
	...
	public void checkAbbr(){
		if(null!=model.getAbbr() && !"".equals(model.getAbbr())){
			boolean isExist = dictBioChemService.isExistByAbbr(model.getAbbr());
			if(!isExist){
				writeJson("true");
			}else{
				writeJson("false");
	...
	public String add() throws Exception {
		...
		model.setOrderNo(dictBioChemService.getNextOrderNo());
		dictBioChemService.save(model);
		return "toList";
	}
	...
	public void checkIsExist()throws Exception{
		...
		if(null != abbr && !"".equals(abbr) ){
			boolean isExist = passagewayService.isExist(abbr,1);
			if(isExist){
	...
	public void delete() {
		...
		if(null!=model.getName() && !"".equals(model.getName())){
			...
			DictBioChem  dictBioChem  = dictBioChemService.getById(model.getName());
			String abbr = dictBioChem.getAbbr();
			boolean isExist = passagewayService.isExist(abbr,1);
			if(isExist){
				json.setSuccess(false);
				json.setMsg("‘"+abbr+"’已设置通道号，无法删除");
			}else{
				dictBioChemService.delete(model.getName());
				json.setSuccess(true);
				json.setMsg("删除成功");
			}
		}else{
	...
	public String editUI() throws Exception {
		if(null!=model.getName() && !"".equals(model.getName())){
			DictBioChem obj  =dictBioChemService.getById(model.getName());
...
//			ActionContext.getContext().getValueStack().push(model);
		...
		DictBioChem dictBioChem =dictBioChemService.getById(model.getName());
		dictBioChem.setAbbr(model.getAbbr());
		dictBioChem.setPrecision(model.getPrecision());
		dictBioChem.setUnit(model.getUnit());
		dictBioChemService.update(dictBioChem);
		return "toList";
	}
	...
	public boolean checkValue(){
		...
		boolean isExistName = dictBioChemService.isExistByName(model.getName());
		if(isExistName){
			return false;
		}
		boolean isExistAbbr = dictBioChemService.isExistByAbbr(model.getAbbr());
		if(isExistAbbr){
			return false;
		}
		return true;
	...
	public boolean checkValue_edit(){
		...
		boolean isExistAbbr = dictBioChemService.isExistByNameAbbr(model.getName(),model.getAbbr());
		if(isExistAbbr){
			return false;
		}
		return true;
	...
	public void moveOrder() {
		...
		if(orderNoPara!=0&&orderNoNext!=0){
			dictBioChemService.moveOeder(orderNoPara, orderNoNext);
			map.put("success", true);
			map.put("msg","移动设置成功");
			DictBioChem currentRow = dictBioChemService.getByOrderNo(orderNoPara);
			DictBioChem nextRow = dictBioChemService.getByOrderNo(orderNoNext);
