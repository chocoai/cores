...
public class TblFileContentSOPAction extends BaseAction<TblFileContentSop> {
	...
	public void save()
	{
		...
		boolean isExistCodeAndVer = isExistSopAndVerInArchive(model.getSopcode(), model.getSopver());
		if(!isExistArchiveCode&&
				((model.getFileRecordId()!=null&&!"".equals(model.getFileRecordId()))//存在sop未归档
						||!isExistCodeAndVer))//
		{
			...
			String key = null;
			TblFileRecord record = null;
			TblFileContentSop fileContentSop = null;
			if(model.getFileRecordId()!=null&&!"".equals(model.getFileRecordId())){//存在的还没有归档的sop
				record = tblFileRecordService.getById(model.getFileRecordId());
				...
				record.setFileFlag(0);//0归档，-1没归档
				
				tblFileRecordService.update(record);
				
				key = model.getFileRecordId();
				fileContentSop = tblFileContentSopService.getById(model.getFileRecordId());
				fileContentSop.setArchiveCode(model.getArchiveCode());
				tblFileContentSopService.update(fileContentSop);
				
			}else if(!isExistCodeAndVer){
	...
	public void importSOPFile()
	{
		...
		if(sopfile2!=null)
		{
			...
			fileContentSop.setSopfile(sopfile);
			fileContentSop.setSopfileName(sopfile2FileName);
			map.put("fileFileName", sopfile2FileName);
		}else {
			map.put("fileFileName",sopfile2FileName);
		}
		tblFileContentSopService.update(fileContentSop);
	...
	public void appendSave()
	{
		...
		if(((model.getFileRecordId()!=null&&!"".equals(model.getFileRecordId()))//存在sop未归档
				||!isExistCodeAndVer))
		{
			//追加归档不用修改fileIndex
	...
	public boolean isExistSopAndVerInArchive(String sopcode, String sopver)
	{
		boolean isExist = tblFileContentSopService.isExistCodeAndVerInArchive(sopcode,sopver);
	...
	public void getNotArchiveByCode()
	{
		...
		if(!isExist)
		{
			List<TblFileContentSop> sops = tblFileContentSopService.getNotArchiveListByCode(model.getSopcode());
			//Integer maxVer=0 ;
			if(sops!=null&&sops.size()>0)
			{
				List<Map<String, Object>> mapList = new ArrayList<Map<String,Object>>();
				//获取parentId
				for(TblFileContentSop fileStudy:sops)
				{
				...
				for(TblFileContentSop fileStudy:sops)
				{
					...
					TblFileIndex fileIndex = fileStudy.getTblFileRecord().getTblFileIndex();
					if(fileIndex!=null)
					{
						...
						map.put("archiveTypeCode","");
						map.put("archiveTypeName", "");
						
						map.put("archiveTitle","");
						map.put("storePosition","");
						map.put("validationFlag", 0);//0：否；1：验证数据
					}
					if (fileStudy.getTblFileRecord()!=null) 
					...
					if(fileStudy.getTblFileRecord().getKeepDate()!=null)
				...
				resultMap.put("msg", "不存在该SOP编号还未归档的SOP！");
			}
	...
	public void getVerByCode()
	{
		...
		if(!isExist)
		{
			TblFileContentSop sop = tblFileContentSopService.getArchiveMaxVerByCode(model.getSopcode());//已经归档的最大版本。
			Integer maxVer=0 ;
			if(sop!=null&&sop.getArchiveCode()!=null)
			{
				try {
				...
				String maxArchiveCode = tblFileIndexService.getMaxCodeByTypeCode(typeCode);
				if(maxArchiveCode!=null)
				{
					map.put("archiveCode",MathUtils.add1ToStringInt(maxArchiveCode));
				}else{
	...
	public void update()
	{
		...
		if(!isExistCodeAndVer)
		{
			...
			{
				writeTblLog(model.getArchiveCode(),fileContentsop.getTblFileRecord().getTblFileIndex().getArchiveTitle()
						,"TblFileContentSop","sopcode",fileContentsop.getTblFileRecord().getFileRecordSn()
					...
					||(fileContentsop.getSopeffectiveDate()!=null&&fileContentsop.getSopeffectiveDate().compareTo(model.getSopeffectiveDate())!=0))
			{
				...
				writeTblLog(model.getArchiveCode(),fileContentsop.getTblFileRecord().getTblFileIndex().getArchiveTitle()
						,"TblFileContentSop","sopeffectiveDate",fileContentsop.getTblFileRecord().getFileRecordSn()
					...
					fileContentsop.getSopinvalidDate()!=null&&fileContentsop.getSopinvalidDate().compareTo(model.getSopinvalidDate())!=0)
			{
				...
				writeTblLog(model.getArchiveCode(),fileContentsop.getTblFileRecord().getTblFileIndex().getArchiveTitle()
						,"TblFileContentSop","sopinvalidDate",fileContentsop.getTblFileRecord().getFileRecordSn()
			...
			{
				writeTblLog(model.getArchiveCode(),fileContentsop.getTblFileRecord().getTblFileIndex().getArchiveTitle()
						,"TblFileContentSop","sopname",fileContentsop.getTblFileRecord().getFileRecordSn()
			...
			{
				writeTblLog(model.getArchiveCode(),fileContentsop.getTblFileRecord().getTblFileIndex().getArchiveTitle()
						,"TblFileContentSop","soptypeCode",fileContentsop.getTblFileRecord().getFileRecordSn()
			...
			{
				writeTblLog(model.getArchiveCode(),fileContentsop.getTblFileRecord().getTblFileIndex().getArchiveTitle()
						,"TblFileContentSop","soptypeName",fileContentsop.getTblFileRecord().getFileRecordSn()
			...
			{
				writeTblLog(model.getArchiveCode(),fileContentsop.getTblFileRecord().getTblFileIndex().getArchiveTitle()
						,"TblFileContentSop","sopver",fileContentsop.getTblFileRecord().getFileRecordSn()
			...
			{
				writeTblLog(model.getArchiveCode(),fileContentsop.getTblFileRecord().getTblFileIndex().getArchiveTitle()
						,"TblFileContentSop","sopflag",fileContentsop.getTblFileRecord().getFileRecordSn()
			...
			/*update的时候位置不会变的
			 * if(!fileIndex.getStorePosition().equals(storePosition)){
				writeTblLog(model.getArchiveCode(),fileIndex.getArchiveTitle(),
			...
			writeES("更新SOP档案", 907, "TblFileContentSop", model.getFileRecordId());
	...
	{
		Map<String, Object> map = new HashMap<String, Object>();
		writeES("删除SOP档案", 908, "TblFileContentSop", model.getFileRecordId());
		...
		tblFileRecordService.update(record);
		String str = "",str1="";
		if(record.getTblFileIndex()!=null)
		{
			str=record.getTblFileIndex().getArchiveCode();
			str1 = record.getTblFileIndex().getArchiveTitle();
		}
		writeTblLog(str,str1,
	...
	public void destroy()
	{
		...
		if(record0.getTblFileIndex()!=null)
		{
			...
			for(TblFileRecord record:recordList)
			{
				...
				String esLinkId = writeES("销毁SOP档案", 921, "TblFileContentSop", record.getFileRecordId());
	...
	public void invalidSOP()
	{
		...
		for(TblFileContentSop contentSop:sopList)
		{
			...
			String esLinkId = writeES("作废SOP档案", 930, "TblFileContentSop", contentSop.getFileRecordId());
			String str = "";
			if(contentSop.getTblFileRecord().getTblFileIndex()!=null)
			{
				str = contentSop.getTblFileRecord().getTblFileIndex().getArchiveTitle();
			}else{
				str = "";
			}
			writeTblLog(contentSop.getArchiveCode(),str,
					"TblFileContentSop","sopinvalidDate",contentSop.getTblFileRecord().getFileRecordSn()
					,""+0,model.getFileRecordId(),""+1,
	...
	public void loadList()
	{
		...
		//fileFlag 0归档 -1未归档 其他的为全部
		Map<String, Object> resultMap =tblFileContentSopService.getByCondition(0,isAll,model.getSopflag(),isNowValid,isInvalid,needChange,changeEndDate,yearNum,yearNumUnit,fileStartDate,fileEndDate,keepEndDate,isDestory,isValid,searchString,page,rows);
