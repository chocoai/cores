...
public interface TblStudyPlanService extends BaseDao<TblStudyPlan> {
	...
	void updateAndSaveTblStudyPlanHis(TblStudyPlan studyPlan,TblApplyRevise applyRevise);
	 ...
	 * @return
	 */
	List<TblStudyPlan> getByStudyType(String studyNo,User user);
	/**
	 * 复制sourceStudyPlanNo的设置到studyNoPara中。
	 */
	boolean copyStudyPlan(String sourceStudyPlanNo,String studyNoPara);
	/**
	 * 根据课题编号和节点名称获取计划日期。
	 */
	Date getGroupingTimeByStudyNoAndNodeName(String sourceStudyPlanNo,String nodeName);
	/**
	 * 根据专题开始日期的范围和模糊查询字段获取专题
	 * @param start
	 * @param end
	 * @param studyName
	 * @return
	 */
	public List<String> getByStartDateAndCondition(Date start,Date end,String studyName);

	/**在YYDB tbplan 中是否存在
	 * @param studyNoPara
	 * @return
	 */
	boolean isExistByStudyNo_YYDB(String studyNoPara);
}
