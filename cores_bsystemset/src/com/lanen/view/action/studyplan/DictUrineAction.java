...
@Controller
@Scope("prototype")
public class DictUrineAction  extends BaseAction<DictUrine>{
	...
	public void loadList(){
		List<DictUrine>  objList=dictUrineService.getAll();
	...
	public void checkName(){
		if(null!=model.getName() && !"".equals(model.getName())){
			boolean isExist = dictUrineService.isExistByName(model.getName());
			if(!isExist){
				writeJson("true");
			}else{
				writeJson("false");
	...
	public void checkAbbr(){
		if(null!=model.getAbbr() && !"".equals(model.getAbbr())){
			boolean isExist = dictUrineService.isExistByAbbr(model.getAbbr());
			if(!isExist){
				writeJson("true");
			}else{
				writeJson("false");
	...
	public String add() throws Exception {
		...
		model.setOrderNo(dictUrineService.getNextOrderNo());
		dictUrineService.save(model);
		return "toList";
	}
	...
	public void checkIsExist()throws Exception{
		...
		if(null != abbr && !"".equals(abbr) ){
			boolean isExist = passagewayService.isExist(abbr,4);
			if(isExist){
	...
	public void delete() {
		Json json = new Json();
		if(null!=model.getName() && !"".equals(model.getName())){
			DictUrine  dictUrine   = dictUrineService.getById(model.getName());
			String abbr = dictUrine.getAbbr();
			boolean isExist = passagewayService.isExist(abbr,4);
			if(isExist){
				json.setSuccess(false);
				json.setMsg("‘"+abbr+"’已设置通道号，无法删除");
			}else{
				dictUrineService.delete(model.getName());
				json.setSuccess(true);
				json.setMsg("删除成功");
			}
	...
	public String editUI() throws Exception {
		if(null!=model.getName() && !"".equals(model.getName())){
			DictUrine obj  =dictUrineService.getById(model.getName());
...
//			ActionContext.getContext().getValueStack().push(model);
		...
		DictUrine DictUrine =dictUrineService.getById(model.getName());
		DictUrine.setAbbr(model.getAbbr());
		DictUrine.setPrecision(model.getPrecision());
		DictUrine.setUnit(model.getUnit());
		dictUrineService.update(DictUrine);
		return "toList";
	}
	...
	public boolean checkValue(){
		...
		boolean isExistName = dictUrineService.isExistByName(model.getName());
		if(isExistName){
			return false;
		}
		boolean isExistAbbr = dictUrineService.isExistByAbbr(model.getAbbr());
		if(isExistAbbr){
			return false;
		}
		return true;
	...
	public boolean checkValue_edit(){
		...
		boolean isExistAbbr = dictUrineService.isExistByNameAbbr(model.getName(),model.getAbbr());
		if(isExistAbbr){
			return false;
		}
		return true;
	...
	public void moveOrder() {
		...
		if(orderNoPara!=0&&orderNoNext!=0){
			dictUrineService.moveOeder(orderNoPara, orderNoNext);
			map.put("success", true);
			map.put("msg","移动设置成功");
			DictUrine currentRow = dictUrineService.getByOrderNo(orderNoPara);
			DictUrine nextRow = dictUrineService.getByOrderNo(orderNoNext);
