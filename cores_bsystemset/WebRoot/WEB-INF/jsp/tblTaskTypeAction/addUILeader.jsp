...
<script type="text/javascript">
    ...
    $(function(){
			...
			tblTaskLeadersTable=$('#tblTaskLeadersTable').datagrid({
				url : sybp()+'/tblTaskTypeLeaderAction_loadtaskLeader.action?taskTypeID='+taskid,
				title:'',
				height:tableHeight,
				width:tableWidth,
				iconCls:'',//'icon-save',
	...
	function onExitButton(){
		...
		window.location.href=sybp()+'/tblTaskTypeAction_list.action?taskIds='+taskid;
	}
	...
	//签字事件
		function afterSign(){
			...
			if(null != rowData){
				$.ajax({
					url:sybp()+'/tblTaskTypeLeaderAction_sign.action',
					type:'post',
					data:{
						id:rowData.id
						},
		...
		function onDelButton(){
			...
			if(null != rowData){
				$.ajax({
					url:sybp()+'/tblTaskTypeLeaderAction_deleteTaskTypeLeader.action',
					type:'post',
					data:{
						id:rowData.id
						},
	    ...
	    function onAddButton(){
			...
			$('#addTaskLeaderDialog').dialog('setTitle','任务总负责人添加');
		...
		function onSaveButton(){
			...
			if(startDate==null || startDate=='' ){
				...
				if(userNames.length > 0){
					$.ajax({
						url:sybp()+'/tblTaskTypeLeaderAction_save.action',
						type:'post',
						data:{
		...
		function afterSign_novail(){
			$.ajax({
				url:sybp()+'/tblTaskTypeLeaderAction_setupInvalid.action',
				type:'post',
...
<div id="layoutMainDiv" class="easyui-tabs" fit="true" border="true" style="display:none">
	<div title="任务总负责人设置"  border="false" style="overflow:hidden;">
		<!-- 工具栏-start -->
		...
		<div style="border-bottom: 1px solid #c8c8c8; height:20px;margin-top:5px;">
			&nbsp;&nbsp;&nbsp;&nbsp;任务类型：<s:property value="{#taskFullName}"/>
		</div>
		<div>
	     ...
	     <%@include file="/WEB-INF/jsp/tblTaskTypeAction/addTaskLeaderDialog.jspf" %>
	</div>
</div>
