...
$.extend($.fn.validatebox.defaults.rules, {  
	 remotett: {   
        ...
        * [validator 远端验证规则]  
        ...
        validator: function(value, param) {   
            var data = {};   
            data[param[1]] = value.replace(/(^\s*)|(\s*$)/g,'');
            var isValidate;
            if(value.replace(/(^\s*)|(\s*$)/g,'') != $(param[2]).val().replace(/(^\s*)|(\s*$)/g,'')){
            	isValidate = $.ajax({   
            		url: param[0],   
            		dataType: "json",   
            		data: data,   
...
$.extend($.fn.validatebox.defaults.rules, {  
	...
	var data = {};   
	data[param[1]] = value.replace(/(^\s*)|(\s*$)/g,'');
	data[param[3]] = $(param[4]).val().replace(/(^\s*)|(\s*$)/g,'');
	data[param[5]] = $(param[6]).val().replace(/(^\s*)|(\s*$)/g,'');
	var isValidate;
	if(value.replace(/(^\s*)|(\s*$)/g,'') != $(param[2]).val().replace(/(^\s*)|(\s*$)/g,'')){
		isValidate = $.ajax({   
			url: param[0],   
