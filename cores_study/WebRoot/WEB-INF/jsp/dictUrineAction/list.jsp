...
<script type="text/javascript">
	...
	$(function(){
		testIndexTable=$('#testIndexTable').datagrid({
			url : sybp()+'/dictUrineAction_loadList.action',
			title:'',
			height: 520,
			//width:560,
			iconCls:'',//'icon-save',
	...
	function onAddToolbarButton(){
		window.location.href=sybp()+'/dictUrineAction_addUI.action';
	}
	//编辑
	function onEditToolbarButton(){
		...
		if(rows.length == 1){
			if(rows.length == 1){
				$.ajax({
					url:sybp()+'/dictUrineAction_checkIsExist.action',
					type:'post',
					data:{
						abbr:rows[0].abbr
					},
					...
					success:function(r){
						if(r && r.success){
							window.location.href=sybp()+'/dictUrineAction_editUI.action?name='+rows[0].name;
						}else{
				...
				success:function(r){
					if(r && r.success){
						$.messager.confirm('提示', '确定删除\' '+rows[0].name+'\' ?', function(r){
							if (r){
								$.ajax({
									url:sybp()+'/dictUrineAction_delete.action',
									type:'post',
									data:{
										name:rows[0].name
									},
	...
	function onUpToolbarButton(){
		...
		if(rows.length == 1){
			...
			if(rowIndex > 0){
				...
				$.ajax({
					url:sybp()+'/dictUrineAction_moveOrder.action',
					type:'post',
					data:{
...
<body>
	<div class="easyui-layout" fit="true" border="false" ">
		<div region="north" border="false"  style="height:26px;">
			<div class="page_tab">
		      <ul>
		          <li class="active"><span><span><a href="javascript:void(0);">尿液指标</a></span></span></li>
		      </ul>
			</div>
		</div>
		<!-- table start -->
