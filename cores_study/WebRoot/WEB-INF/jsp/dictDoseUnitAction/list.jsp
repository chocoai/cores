...
<script type="text/javascript">
	...
	$(function(){
		testIndexTable=$('#testIndexTable').datagrid({
			url : sybp()+'/dictDoseUnitAction_loadList.action',
			title:'',
			height: 520,
			//width:560,
			iconCls:'',//'icon-save',
	...
	function onAddToolbarButton(){
		window.location.href=sybp()+'/dictDoseUnitAction_addUI.action';
	}
	//编辑
	function onEditToolbarButton(){
		var rows = testIndexTable.datagrid('getSelections');
		if(rows.length == 1){
			window.location.href=sybp()+'/dictDoseUnitAction_editUI.action?name='+rows[0].name;
	...
	function onDelToolbarButton(){
		...
		if(rows.length == 1){
			...
			$.messager.confirm('提示', '确定删除\' '+rows[0].name+'\' ?', function(r){
				if (r){
					$.ajax({
						url:sybp()+'/dictDoseUnitAction_delete.action',
						type:'post',
						data:{
							name:rows[0].name
						},
	...
	function onUpToolbarButton(){
		...
		if(rows.length == 1){
			...
			if(rowIndex > 0){
				...
				$.ajax({
					url:sybp()+'/dictDoseUnitAction_moveOrder.action',
					type:'post',
					data:{
...
<body>
	<div class="easyui-layout" fit="true" border="false" ">
		<div region="north" border="false"  style="height:26px;">
			<div class="page_tab">
		      <ul>
		          <li class="active"><span><span><a href="javascript:void(0);">剂量单位</a></span></span></li>
		      </ul>
			</div>
		</div>
		<!-- table start -->
