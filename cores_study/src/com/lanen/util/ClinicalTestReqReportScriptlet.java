...
public class ClinicalTestReqReportScriptlet extends JRAbstractScriptlet {
	...
	public void afterPageInit() throws JRScriptletException {
		...
		String testIndex4="";
		//拼接每一个检测项目的字符串
		Set<TblClinicalTestReqIndex> tblClinicalTestReqIndexs = (Set<TblClinicalTestReqIndex>) StaticDate.STATIC_DATE_MAP.get("tblClinicalTestReqIndexs");
...
//		InputStream   imageIn   =   new   FileInputStream(imageFile);//图片一个数据流
		InputStream   imageIn    = (InputStream) StaticDate.STATIC_DATE_MAP.get("logoImage");
		setVariableValue( "logoImage",imageIn);//获得为图片设置的参数，把图片传给这个参数 
	}
