...
public interface TblAnatomyReqService extends BaseDao<TblAnatomyReq> {
	...
	/**变更申请（把申请置为可编辑状态，submitFlag = 2，提交人和提交日期置为null）
	 * 若第一变更，保存解剖申请记录（共5个表） 
	 * 若非第一变更，仅submitFlag = 2，提交人和提交日期置为null
	 * @param tblAnatomyReq
	 */
	void change(TblAnatomyReq tblAnatomyReq);
	
	boolean isExistHis(String studyNo,int reqNo);
	 ...
	 * @return
	 */
	List<String> getErrorAnimalCodeList(String studyNo,
			List<String> animalCodeList);

	/**查询已被解剖的脏器列表列表
	 * @param studyNo
	 * @param visceraCodeListCheck
	 * @return
	 */
	List<String> getErrorCheckVisceraCodeList(String studyNo,
			List<String> visceraCodeListCheck);

	/**查询已被固定的脏器列表列表
	 * @param studyNo
	 * @param visceraCodeListFixed
	 * @return
	 */
	List<String> getErrorFixedVisceraCodeList(String studyNo,
			List<String> visceraCodeListFixed);

	/**查询已被称重的脏器列表列表
	 * @param studyNo
	 * @param visceraCodeListWeigh
	 * @return
	 */
	List<String> getErrorWeighVisceraCodeList(String studyNo,
	 ...
	 * @return
	 */
	TblAnatomyReqHis getByHisId(String reqId);
	 ...
	 * @return
	 */
	int getAnatomyRsn(String studyNo, Integer reqNo);
