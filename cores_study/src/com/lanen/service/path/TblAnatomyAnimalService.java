...
//	List<Map<String, Object>> getListBySessionIdList2(List<String> sessionIdList);
	...
	/**根据任务id列表会话类型，查询待解剖或称重或固定的动物列表(mapList);（老版本）
	 * Map中有studyNo,anatomyNum,animalCode,gender,animalId ,taskId,anatomyOperator
	 * @param taskIdList
	 * @param sessionType
	 * @return
	 */
	List<Map<String,Object>> getMapListByTaskIdListSessionType(List<String> taskIdList,Integer sessionType);
	...
	/**保存或更新解剖动物信息,返回map（map中对应字段有：taskId,studyNo，
	 * animalCode，realName操作者 ，visceraWeightFinishSign称重完成签字
	 * ，visceraFixedFinishSign固定完成签字，anatomyCheckFinishFlag剖检完成标识,autolyzeFlag自溶标识,出错时msg存放错误信息，否则为""）(老版本)
	 * @param taskId
	 * @param sessionId
	 * @param sessionType
	 * @param animalCode
	 ...
	 * @return
	 */
	Map<String, Object> saveOrUpdate(String taskId, String sessionId, Integer sessionType, String animalCode, String userName, Date anatomyBeginTime);
	 ...
	 * @param studyNo
	 * @param animalCode
	 * @param fixedWeightFlag 固定 前  后
	 * @return
	 */
	boolean isWeightFinish(String studyNo, String animalCode,
			int fixedWeightFlag);
